...
Classroom live code to accompany the Functional Programming Industry Prep topic.
...
Implement the functions in `functional.py` using only functional programming techniques. This means:
- NO FOR OR WHILE LOOPS ALLOWED
- NO COMPREHENSIONS ALLOWED

You can assume valid, non-empty input. Focus on the core logic of the challenge, not on validating inputs.
...
You may need to make your own additional functions to pass in to some of these functions. These may be created either as stand-alone definitions, or as lambdas. Discuss the pros and cons of each approach.